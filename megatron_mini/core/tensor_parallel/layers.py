...
class LinearWithGradAccumulationAndAsyncCommunication(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        #     print(f"rank_{torch.distributed.get_rank()}, grad_output: {grad_output.shape}", flush=True)
        
        # Convert the tensor shapes to 2D for execution compatibility, TODO: check reasonable for contiguous() methods
        grad_output = grad_output.contiguous().view(grad_output.shape[0] * grad_output.shape[1],
                                        grad_output.shape[2])
